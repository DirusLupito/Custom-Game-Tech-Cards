...
//set by lobby settings
var eradicationModeUnitTypes = ["Commander"]
...
function tickDefeatState(check_for_resurrection) {
    ...
    _.forEach(armies, function (army) {
        ...
        else if(game_options.eradication_mode) {
            var everyone_surrendered = army.players.length > 0 && _.every(army.players, function (player) {
                return player.has_surrendered;
            });

            alive = !everyone_surrendered && army.sim.hasUnitOfAnyType(eradicationModeUnitTypes)
        }
        else {
            alive = _.some(army.commanders, function (commander) { return !commander.dead; });
        }
...
exports.enter = function (config) {
    ...
    if (game_options.eradication_mode) {
        //console.log("================== Loading eradication mode unit types based on game options ================");
        if (game_options.eradication_mode_sub_commanders ) {
            //console.log("Adding SupportCommander to eradication mode unit types");
            eradicationModeUnitTypes.push("SupportCommander")
        }
        if (game_options.eradication_mode_factories) {
            //console.log("Adding Factory to eradication mode unit types");
            eradicationModeUnitTypes.push("Factory")
        }
        if (game_options.eradication_mode_fabricators) {
            //console.log("Adding Fabber to eradication mode unit types");
            eradicationModeUnitTypes.push("Fabber")
        }
        console.log("eradication mode unit types loaded: " + eradicationModeUnitTypes);
    }
