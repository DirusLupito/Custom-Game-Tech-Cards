...
var communityServersBeaconTimer = false;

server.updateCommunityServersBeacon = function()
{
    var beacon = server.beacon;

    if (beacon)
        server.beacon = beacon;
}

server.startCommunityServersBeacon = function()
{
    console.log ('Starting community servers beacon');
    communityServersBeaconTimer = setInterval(server.updateCommunityServersBeacon, 5000);
}

server.stopCommunityServersBeacon = function()
{
    if (communityServersBeaconTimer) {
        clearInterval(communityServersBeaconTimer);
        communityServersBeaconTimer = false;
    }
}
